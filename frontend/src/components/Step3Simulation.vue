...
const doStartSimulation = async () => {
  ...
  // Reset all state first so the previous simulation cannot affect this run
  resetAllState()
...
// Track the previous round for each platform to detect changes and output logs
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect whether simulation has completed (via runner_status or platform completion state)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Extra check: if backend has not updated runner_status yet but platforms already report completion
      // Determine by checking twitter_completed and reddit_completed states
...
const checkPlatformsCompleted = (data) => {
  ...
  // If at least one platform completed, check whether all enabled platforms completed
  // Determine whether a platform was enabled via actions_count (if count > 0 or running was ever true)
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Do not auto-scroll so users can freely inspect the timeline
