  ...
  >
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        ...
        <!-- Card description (shows the full simulation requirement) -->
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <div class="modal-section">
                ...
                <div class="modal-empty" v-else>No related files yet</div>
              </div>
            </div>
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 "Start Simulation" and Step5 "Deep Interaction" must be launched during a run and do not support history playback</span>
            </div>
          </div>
        </div>
      </Transition>
...
const containerStyle = computed(() => {
  ...
  // Calculate required height: rows * card height + (rows - 1) * gap + a small bottom gap
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate card count for the current row so each row is centered
    ...
    const x = offset * 35
    // Adjust the starting position to stay near the title with proper spacing
...
// Smart route navigation: go directly to report for completed/reporting, otherwise go to the project process page
...
// Load historical projects (with polling)
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // If animating, only record state and handle it after the animation ends
        if (isAnimating) return
        
        // If target state matches current state, no handling is needed
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // Debounce state switching to prevent rapid flicker
        // Use a shorter delay when expanding (50ms) and a longer delay when collapsing (200ms) for stability
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check whether the pending state still needs execution (it may have been overridden by later scrolling)
          ...
          // Release the lock after animation and check for pending state changes
          setTimeout(() => {
            isAnimating = false
            
            // After animation ends, check for a new pending state
    ...
    },
    {
      ...
      // Adjust rootMargin so the viewport bottom shrinks upward, requiring more scrolling before expansion is triggered
...
/* Use a CSS background pattern to create a fixed-spacing square grid */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* Position from the top-left; when height changes, expand only at the bottom without affecting existing grid positions */
...
/* ===== Empty state (overrides existing styles and adds child styles) ===== */
