    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Create a simulation instance and fetch the simulated-world parameter template
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Using context, tools are called automatically to organize entities and relationships from the knowledge graph, initialize simulated individuals, and give them distinctive behavior and memories based on real-world seeds
          </p>
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            ...
            <div class="stat-card">
              ...
              <span class="stat-label">Current Real-Seed Related Topics</span>
            </div>
          </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            The LLM intelligently sets world time speed, recommendation algorithms, each individual's active periods, posting frequency, event triggers, and other parameters based on simulation requirements and real-world seeds
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Based on the narrative direction, initial activation events and hot topics are generated automatically to guide the simulated world's initial state
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Simulation round configuration - shown only after config generation completes and round count is calculated -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">Simulation Round Settings</span>
                <span class="section-desc">MiroFish automatically plans a simulation of <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours. Each round represents <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes of real time elapsed</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>For 100 Agents: estimated time about {{ Math.round(customMaxRounds * 0.6) }} minutes</span>
                  </div>
                </div>
              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">For a first run, switching to Custom mode and reducing simulation rounds is strongly recommended for a faster preview and lower error risk ➝</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div v-if="selectedProfile" class="profile-modal-overlay" @click.self="selectedProfile = null">
        ...
        <div class="modal-body">
          ...
          <div class="modal-info-grid">
            <div class="info-item">
              <span class="info-label">Event-Visible Age</span>
              <span class="info-value">{{ selectedProfile.age || '-' }} years old</span>
            </div>
            <div class="info-item">
              <span class="info-label">Event-Visible Gender</span>
            ...
            <div class="info-item">
              <span class="info-label">Event-Visible MBTI</span>
          ...
          <div class="modal-section" v-if="selectedProfile.interested_topics?.length">
            <span class="section-label">Real-Seed Related Topics</span>
...
// Log deduplication: record the last key message output
let lastLoggedMessage = ''
let lastLoggedProfileCount = 0
let lastLoggedConfigStage = ''

// Simulation round configuration
const useCustomRounds = ref(false) // use automatically configured rounds by default
...
watch(currentStage, (newStage) => {
  if (newStage === '\u751f\u6210Agent\u4eba\u8bbe' || newStage === 'generating_profiles') {
    ...
    phase.value = 2 // still belongs to the config phase
  }
})

// Calculate automatically generated rounds from config (without hardcoded default)
const autoGeneratedRounds = computed(() => {
  ...
  // Ensure max rounds is not below 40 (recommended value) to avoid abnormal slider range
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    // User custom round count; pass max_rounds
    params.maxRounds = customMaxRounds.value
    addLog(`Starting simulation with custom rounds: ${customMaxRounds.value} rounds`)
  } else {
    // User keeps the auto-generated round count; do not pass max_rounds
    addLog(`Starting simulation with automatically configured rounds: ${autoGeneratedRounds.value} rounds`)
  }
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog('Detected completed preparation work; using it directly')
      ...
      // Immediately set Expected Agent Total from the prepare API response
    ...
    if (res.success && res.data) {
      ...
      if (data.config_generated && data.config) {
        ...
        if (data.config.time_config) {
          ...
          addLog(`Time Config: each round ${tc.minutes_per_round} minutes, total ${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)} rounds`)
        }
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        addLog('Config is generating; start polling and waiting...')
        startConfigPolling()
      }
    }
  } catch (err) {
