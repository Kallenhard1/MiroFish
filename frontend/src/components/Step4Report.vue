              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- Show special hint for the final answer -->
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract statistics - match the "related prediction facts: X items" format
    ...
    // Extract subquestions fully without limiting count
    ...
    // Extract key facts fully without limiting count
    ...
    // Extract core entities fully, including summary and related fact count
    ...
    // Extract relationship chains fully without limiting count
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract current valid facts fully without limiting count
    ...
    // Extract historical/expired facts fully without limiting count
    ...
    // Extract involved entities fully without limiting count
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Extract reasons for interviewee selection
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Example: 1. **alumnus_345 (index=1)**: as a WHU alumnus...
        ...
        if (name) {
          ...
          // Continuation line for the reason (excluding closing summary paragraphs)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (supports old format with only one platform marker)
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer exists; copy as default display only when it is not placeholder text
          ...
          // No platform markers (very old format); use the entire text as the answer
      ...
      if (quotesMatch) {
        ...
        // Compatibility handling for Chinese-formatted backend output
        if (!quoteMatches) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Extract related facts fully without limiting count
...
/* Dark gray; does not change with status */
