    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? 'GraphRAG long- and short-term memory updating in real time' : 'Updating in real time...' }}
        </div>
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">A small amount of content is still being processed. Manual graph refresh is recommended later.</span>
...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Show finished hint when switching from simulating to not simulating
...
const renderGraph = () => {
  ...
  // Process edge data and calculate counts and indices for each node pair
  const edgePairCount = {}
  ...
  // Record which edge index is being processed for each node pair
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    // Determine whether edge direction matches the normalized direction (source UUID < target UUID)
    ...
    if (totalCount > 1) {
      ...
      // Increase curvature range with edge count; more edges means a larger range
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // Flip curvature if edge direction is opposite to normalized direction
      // This keeps all edges distributed in the same reference frame and prevents overlap from direction differences
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Dynamically adjust distance based on the edge count between this node pair
    ...
    // Add center gravity so independent node clusters gather near the center
  ...
  const getLinkPath = (d) => {
    ...
    // Offset perpendicular to the connection direction, calculated by distance ratio so the curve is clearly visible
    // More edges means a larger offset ratio relative to distance
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Only record position without restarting simulation (distinguish click from drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Detect whether dragging really started (movement exceeds threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Restart simulation only after real dragging is first detected
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Let simulation gradually stop only if real dragging occurred
        if (d._isDragging) {
  ...
  simulation.on('tick', () => {
    ...
    linkLabels.each(function(d) {
      ...
      d3.select(this)
        .attr('x', mid.x)
        .attr('y', mid.y)
        .attr('transform', '') // Remove rotation and keep horizontal
    })
