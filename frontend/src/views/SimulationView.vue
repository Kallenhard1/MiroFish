...
const handleNextStep = (params = {}) => {
  ...
  // If custom rounds are provided, pass them through query params
  if (params.maxRounds) {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      if (simRes.success && simRes.data?.status === 'running') {
        addLog('Detected simulation status as running; stopping it...')
        await forceStopSimulation()
      }
    }
  } catch (err) {
...
onMounted(async () => {
  ...
  // Check and close any running simulation (when the user returns from Step 3)
