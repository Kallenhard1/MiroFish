...
def generate_ontology():
    ...
    try:
        ...
        logger.info(f"Text extraction complete, {len(all_text)} characters in total")
...
def build_graph():
    """
    API 2: Build a graph from project_id
    ...
    try:
        ...
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "Graph is currently building. Please do not submit again. To force a rebuild, add force: true",
        ...
        return jsonify({
            ...
            "data": {
                "project_id": project_id,
                "task_id": task_id,
                "message": "Graph build task started. Query progress through /task/{task_id}"
            }
        })
