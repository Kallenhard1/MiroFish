...
This module wraps automatic page traversal (with single-page retries) and returns complete lists transparently to callers.
"""
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """Request one page and retry with exponential backoff on failure. Only transient network/IO errors are retried."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """Fetch graph nodes with pagination, returning at most max_items items (default 2000). Each page request includes retries."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Fetch all graph edges with pagination and return the complete list. Each page request includes retries."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
