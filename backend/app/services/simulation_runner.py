...
# flag: cleanup handler has been registered
...
class SimulationRunState:
    ...
    def add_action(self, action: AgentAction):
        """Prepend action to recent actions list."""
...
class SimulationRunner:
    ...
    _stdout_files: Dict[str, Any] = {}  # stdout file handle
    _stderr_files: Dict[str, Any] = {}  # stderr file handle
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # max simulation rounds (optional, truncates long simulations)
        enable_graph_memory_update: bool = False,  # whether to push activity updates to Zep graph
        ...
        Args:
            ...
            max_rounds: max simulation rounds (optional, truncates long simulations)
            enable_graph_memory_update: whether to dynamically push agent activity to Zep graph
        ...
        # if graph memory update enabled, create the updater
        ...
        # select script to run (located in backend/scripts/)
        if platform == "twitter":
        ...
        try:
            ...
            # add max-rounds to command args if specified
            if max_rounds is not None and max_rounds > 0:
                cmd.extend(["--max-rounds", str(max_rounds)])

            # write to a log file instead of a pipe to avoid buffer-full blocking
            main_log_path = os.path.join(sim_dir, "simulation.log")
            main_log_file = open(main_log_path, 'w', encoding='utf-8')

            # set subprocess env to enforce UTF-8 encoding on Windows
            # fixes third-party libs (e.g. OASIS) that open files without specifying encoding
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'  # Python 3.7+: forces all open() calls to default to UTF-8
            env['PYTHONIOENCODING'] = 'utf-8'  # ensure stdout/stderr use UTF-8

            # set cwd to simulation dir so databases and outputs land there
            # start_new_session=True creates a new process group, allowing os.killpg to terminate all children
            process = subprocess.Popen(
                cmd,
                cwd=sim_dir,
                stdout=main_log_file,
                stderr=subprocess.STDOUT,  # stderr merged into stdout file
                text=True,
                encoding='utf-8',
                bufsize=1,
            ...
            cls._stderr_files[simulation_id] = None  # stderr merged into stdout file
    ...
    def _read_action_log(
        ...
        """
        Read entries from an action log file starting at the given byte position.

        Args:
            log_path: path to log file
            position: byte offset from last read
            state: run state object to update
                    ...
                    if line:
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                # simulation_end event: mark platform as done
                                if event_type == "simulation_end":
                                    ...
                                    # check whether all active platforms have finished:
                                    # single-platform run → only check that platform
                                    # dual-platform run → both must be done
                                ...
                                # update round info from round_end events
                                    ...
                                    # overall simulated hours = max across both platforms
                            ...
                            # update overall round
                            if action.round_num and action.round_num > state.current_round:
                                state.current_round = action.round_num

                            # push activity to Zep if graph memory update is enabled
                            if graph_updater:
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        """
        Check whether all active platforms have finished their simulation.

        A platform is considered active if its actions.jsonl file exists.

        Returns:
            True if all active platforms are done.
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, state.simulation_id)
        twitter_log = os.path.join(sim_dir, "twitter", "actions.jsonl")
        reddit_log = os.path.join(sim_dir, "reddit", "actions.jsonl")

        # determine which platforms are active based on file presence
        twitter_enabled = os.path.exists(twitter_log)
        reddit_enabled = os.path.exists(reddit_log)

        # return False if any active platform is not yet done
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        if IS_WINDOWS:
            ...
            # start_new_session=True means pgid == main process pid
            ...
            try:
                ...
                # escalate to SIGKILL if still alive after timeout
    ...
    def _read_actions_from_file(
        ...
        Args:
            ...
            agent_id: only return actions for this agent ID
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # platform: prefer field in record, fall back to default
    ...
    def get_all_actions(
        ...
        Returns:
            full action list sorted by timestamp descending
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        actions = []
        ...
        if not platform or platform == "twitter":
            actions.extend(cls._read_actions_from_file(
                twitter_actions_log,
                default_platform="twitter",  # fill in missing platform fields
        ...
        if not platform or platform == "reddit":
            actions.extend(cls._read_actions_from_file(
                reddit_actions_log,
                default_platform="reddit",  # fill in missing platform fields
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            ))
        ...
        if not actions:
            ...
            actions = cls._read_actions_from_file(
                actions_log,
                default_platform=None,  # legacy files already have platform field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            )
    ...
    def get_timeline(
        ...
        """
        actions = cls.get_actions(simulation_id, limit=20)
    ...
    def get_agent_stats(cls, simulation_id: str) -> List[Dict[str, Any]]:
        ...
        Returns:
            list of agent stats sorted by total action count descending
        """
        actions = cls.get_actions(simulation_id, limit=20)
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        ...
        logger.info(f"Simulation logs cleaned: {simulation_id}, deleted: {cleaned_files}")
    ...
    # guard flag against duplicate cleanup
    _cleanup_done = False
    ...
    def cleanup_all_simulations(cls):
        ...
        Called on server shutdown to ensure all child processes are killed.
        """
        ...
        # skip if there's nothing to clean — avoid noise in logs
        ...
        # copy dict to avoid modifying while iterating
            ...
            try:
                if process.poll() is None:  # process still running
                    ...
                    try:
                        ...
                        # process may already be gone, fall back to direct termination
                        try:
                            process.terminate()
        ...
        logger.info("All simulation processes cleaned up")
    ...
    def register_cleanup(cls):
        ...
        Call at Flask app startup to ensure all simulation processes are terminated on shutdown.
        """
        ...
        # In Flask debug mode, only register in the reloader child process (the one actually serving)
        # WERKZEUG_RUN_MAIN=true identifies the reloader child
        # In non-debug mode the env var is absent but we still need to register
        is_reloader_process = os.environ.get('WERKZEUG_RUN_MAIN') == 'true'
        is_debug_mode = os.environ.get('FLASK_DEBUG') == '1' or os.environ.get('WERKZEUG_RUN_MAIN') is not None

        # debug mode: only register in reloader child; non-debug: always register
        if is_debug_mode and not is_reloader_process:
            _cleanup_registered = True  # prevent child process from retrying
            return
        ...
        def cleanup_handler(signum=None, frame=None):
            """Signal handler: clean up simulations then forward to original handler."""
            ...
            if signum == signal.SIGINT and callable(original_sigint):
                ...
                # SIGHUP: sent when the controlling terminal closes
                if callable(original_sighup):
                    original_sighup(signum, frame)
                else:
                    sys.exit(0)
            else:
                # SIG_DFL or similar: fall back to default behavior
    ...
    def get_env_status_detail(cls, simulation_id: str) -> Dict[str, Any]:
        ...
        Returns:
            status dict with keys: status, twitter_available, reddit_available, timestamp
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: target platform (optional)
                ...
                - None: interview on both platforms in a dual-platform simulation, returns merged result
        ...
        if not ipc_client.check_env_alive():
            raise ValueError(f"Simulation environment not running or already closed — cannot interview: {simulation_id}")
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: default platform (optional, overridden per item)
                ...
                - None: interview on both platforms per agent in dual-platform simulations
            timeout: timeout in seconds
    ...
    def interview_all_agents(
        ...
        """
        Interview all agents with the same question (global interview).
    ...
    def close_simulation_env(
        ...
        Sends a close-env command to the simulation so it exits command-waiting mode gracefully.
        ...
        try:
            ...
            # timeout likely means the environment is already shutting down
            return {
                "success": True,
                "message": "Close-env command sent (response timed out — environment may be shutting down)"
            }
    ...
    def _get_interview_history_from_db(
        ...
        """Fetch interview history from a single platform database."""
        import sqlite3
