...
Runs Twitter and Reddit simulations concurrently using the same configuration file

Features:
- Dual-platform (Twitter + Reddit) parallel simulation
- Keep the environment open after simulation and enter command-wait mode
...
# This fixes third-party OASIS libraries reading files without specifying an encoding
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    # Reconfigure standard output stream to UTF-8 (solves console text encoding issues)
    if hasattr(sys.stdout, 'reconfigure'):
        sys.stdout.reconfigure(encoding='utf-8', errors='replace')
    if hasattr(sys.stderr, 'reconfigure'):
        sys.stderr.reconfigure(encoding='utf-8', errors='replace')
    
    # Force the default encoding (affects the default encoding for open())
    # Note: this should be set when Python starts; setting it at runtime may not take effect
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None, 
        ...
        """
        Wrap open() so text mode uses UTF-8 by default.
        This fixes third-party libraries such as OASIS reading files without specifying an encoding.
        """
        # Set the default encoding only for text mode (non-binary) when no encoding is specified
...
# Load the project-root .env file (contains LLM_API_KEY and other settings)
...
class MaxTokensWarningFilter(logging.Filter):
    """Filter camel-ai max_tokens warnings (we intentionally omit max_tokens and let the model decide)"""
...
# Add the filter as soon as the module loads so it takes effect before camel code runs
...
def disable_oasis_logging():
    ...
    OASIS logs are too verbose (recording every agent observation and action), so we use our own action_logger
    """
...
# Available Twitter actions (excludes INTERVIEW; INTERVIEW can only be triggered manually through ManualAction)
TWITTER_ACTIONS = [
...
# Available Reddit actions (excludes INTERVIEW; INTERVIEW can only be triggered manually through ManualAction)
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
...
class ParallelIPCHandler:
    ...
    async def _interview_single_platform(self, agent_id: int, prompt: str, platform: str) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary containing the result, or a dictionary containing error
        """
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Specified platform (optional)
                ...
                - None/not specified: Interview both platforms and return merged results
        ...
        if success_count > 0:
            ...
            print(f"  Interview completed: agent_id={agent_id}, successful platform count={success_count}/{len(platforms_to_interview)}")
            return True
        else:
    ...
    def _get_interview_result(self, agent_id: int, platform: str) -> Dict[str, Any]:
        ...
        try:
            ...
            cursor.execute("""
                ...
                FROM trace
                WHERE action = or AND user_id = or
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    """
    Get agent_id -> entity_name mapping from simulation_config.json.
    
    This allows displaying the real entity names in actions.jsonl instead of codes like "Agent_0".
...
def fetch_new_actions_from_db(
    ...
    """
    Fetch new action records from the database and enrich with complete context information.
    
    Args:
        db_path: Database file path
        last_rowid: Largest rowid value read last time (uses rowid instead of created_at because created_at formats differ by platform)
        agent_names: agent_id -> agent_name mapping
        
    Returns:
        (actions_list, new_last_rowid)
        - actions_list: Action list where each item contains agent_id, agent_name, action_type, and action_args (with context)
...
def _enrich_action_context(
    ...
    try:
        ...
        # Repost: enrich original post content and author
            ...
            if new_post_id:
                # The repost original_post_id points to the original post
                cursor.execute("""
                    SELECT original_post_id FROM post WHERE post_id = or
                """, (new_post_id,))
                row = cursor.fetchone()
                if row and row[0]:
        ...
        # Quote post: enrich original post content, author, and quote comment
            ...
            # Get the quote post comment content (quote_content)
            if new_post_id:
                cursor.execute("""
                    SELECT quote_content FROM post WHERE post_id = or
                """, (new_post_id,))
                row = cursor.fetchone()
                if row and row[0]:
                    action_args['quote_content'] = row[0]
        
        # Follow user: enrich the followed user name
        elif action_type == 'FOLLOW':
            follow_id = action_args.get('follow_id')
            if follow_id:
                # Get followee_id from the follow table
                cursor.execute("""
                    SELECT followee_id FROM follow WHERE follow_id = or
                """, (follow_id,))
                row = cursor.fetchone()
                if row:
                    followee_id = row[0]
        ...
        # Mute user: enrich the muted user name
        ...
        # Create comment: enrich the commented post information
        ...
        # Enriching context failure does not affect main workflow
...
def _get_post_info(
    ...
    try:
        ...
        if row:
            ...
            # Prefer names from the agent_names mapping
            author_name = ''
            if agent_id is not None and agent_id in agent_names:
                author_name = agent_names[agent_id]
            elif user_id:
                # Get the name from the user table
                cursor.execute("SELECT name, user_name FROM user WHERE user_id = or", (user_id,))
...
def _get_user_name(
    ...
    try:
        cursor.execute("""
            SELECT agent_id, name, user_name FROM user WHERE user_id = or
        """, (user_id,))
        row = cursor.fetchone()
        if row:
            agent_id = row[0]
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    """
    # Check whether boost configuration exists
    ...
    # If .env does not have model name, use config as fallback
    if not llm_model:
...
def get_active_agents_for_round(
    ...
    """Determine which Agents are active this round based on time and configuration"""
...
async def run_twitter_simulation(
    ...
    # Get real Agent name mapping from the configuration file (use entity_name instead of the default Agent_X)
    agent_names = get_agent_names_from_config(config)
    # If the config has no name for an agent, use the OASIS default name
    ...
    # Record the start of round 0 (initial event phase)
    if action_logger:
    ...
    for round_num in range(total_rounds):
        ...
        # Record round start whether or not there are active agents
        if action_logger:
        ...
        # Fetch and log actions actually executed from the database
...
async def main():
    ...
    # Initialize logging configuration (disable OASIS logs and clean up old files)
    ...
    # Store simulation results from both platforms
        ...
        try:
            while not _shutdown_event.is_set():
                ...
                try:
                    ...
                    pass  # Continue loop after timeout
...
def setup_signal_handlers(loop=None):
    """
    Set up signal handlers to exit correctly on SIGTERM/SIGINT
    ...
    2. Give the program a chance to clean up resources normally (close databases, environments, etc.)
    3. Then exit
    """
