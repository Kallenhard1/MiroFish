...
This script reads parameters from the configuration file to run the simulation fully automatically
...
class TwitterSimulationRunner:
    ...
    def _create_model(self):
        ...
        Consistently use configuration from the project-root .env file (highest priority):
        - LLM_API_KEY: API key
        ...
        # If .env does not contain it, use config as a fallback
        if not llm_model:
    ...
    def _get_active_agents_for_round(
        ...
        # Calculate activation probability based on each Agent configuration
        candidates = []
...
async def main():
    ...
    # Initialize logging configuration (use fixed filenames and clean up old logs)
...
def setup_signal_handlers():
    ...
    Give the program a chance to clean up resources normally (close databases, environments, etc.)
    """
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            ...
            # Force exit only when the signal is received repeatedly
            print("Force exiting...")
            sys.exit(1)
